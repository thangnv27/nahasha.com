...
function share_story_network_link($network, $id, $simptips = true)
{
	...
	$networks = array(
		...
		'lin' => array(
			'url'		=> 'https://linkedin.com/shareArticle?mini=true&amp;url=' . get_permalink() . '&amp;title=' . esc_attr( get_the_title() ),
			'tooltip'	=> __('LinkedIn', 'aurum'),
			'icon'		 => 'linkedin'
		),

		'pi' => array(
			'url'		=> 'https://pinterest.com/pin/create/button/?url=' . get_permalink() . '&amp;description=' . esc_attr( get_the_title() ) . '&amp;' . ($id ? ('media=' . wp_get_attachment_url( get_post_thumbnail_id($id) )) : ''),
	...
	<a class="<?php echo $network_entry['icon']; ?>" href="<?php echo $network_entry['url']; ?>" target="_blank">
		...
		<i class="entypo entypo-<?php echo $network_entry['icon']; ?>"></i>
		<?php else: ?>
...
if( ! function_exists( 'get_field' ) && ! in_array('advanced-custom-fields/acf.php', apply_filters('active_plugins', get_option('active_plugins'))) && ! is_admin())
{
...
function aurum_show_header_cart_icon( $size = array( 24, 24 ) ) {
	...
	<section class="cart-info">
		<a class="cart-counter<?php echo $cart_items_count ? ' has-notifications' : ''; echo $cart_items_count == 0 ? " cart-zero" : ''; echo $shop_cart_counter_ajax ? ' cart-counter-ajax' : ''; ?>" href="<?php echo WC()->cart->get_cart_url(); ?>">
